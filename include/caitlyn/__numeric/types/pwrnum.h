...
{
public:
    using sign_flag = pwrint_t::sign_flag;
    using string_type = pwrint_t::string_type;
    using size_type = pwrint_t::size_type;

public:
    pwrnum_t() = default;

    pwrnum_t(const double value) : pwrnum_t{std::to_string(value)}
    {
    }

    pwrnum_t(const char* number) : pwrnum_t{string_type{number}}
    {
    }

    pwrnum_t(const string_type& number)
    {
        if (number.is_empty())
        {
            throw std::invalid_argument{"Invalid number format"};
        }
        size_t point_pos = number.find(def::full_stop);
        if (point_pos == string_type::npos)
        {
            integer_part_ = pwrint_t{number};
        }
        else
        {
            string_type integer_str = number.substr(0, point_pos);
            string_type fractional_str = number.substr(point_pos + 1);
...
public:
    pwrnum_t add(const pwrnum_t& other) const
    {
        ...
        {
            carry = true;
            fractional_result = fractional_result.subtract(pwrint_t{"10"}.multiply(this_fractional.size()));
        }
        ...
        return pwrnum_t{integer_result.to_string() + "." + fractional_result.to_string()};
    }
    ...
    pwrnum_t subtract(const pwrnum_t& other) const
    {
        ...
        {
            borrow = true;
            this_fractional = this_fractional.add(pwrint_t{"10"}.multiply(this_fractional.size()));
        }
        const pwrint_t fractional_result = this_fractional.subtract(other_fractional);
    ...
    pwrnum_t multiply(const pwrnum_t& other) const
    {
        ...
        // decimal point
        const string_type this_full = integer_part_.to_string() + fractional_part_.to_string();
        const string_type other_full = other.integer_part_.to_string() + other.fractional_part_.to_string();
        ...
        const pwrint_t result_full = pwrint_t{this_full}.multiply(pwrint_t{other_full});

        // The total number of fractional digits in the result
        const size_t total_frac_size = this_frac_size + other_frac_size;

        // Convert the result back to a string
        string_type result_str = result_full.to_string();

        // Insert the decimal point at the correct position
        if (total_frac_size >= result_str.size())
        {
            result_str.insert(0, string_type(total_frac_size - result_str.size() + 1, '0'));
        }
    ...
    pwrnum_t divide(const pwrnum_t& other) const
    {
        ...
        const pwrint_t result_full = pwrint_t{this_full}.divide(pwrint_t{other_full});
...
public:
    ...
    {
        return integer_part_.equal(other.integer_part_) && fractional_part_.equal(other.fractional_part_) &&
               is_negative_ == other.is_negative_;
    }
    ...
    {
        string_type result = integer_part_.to_string();
        if (!fractional_part_.to_string().is_empty() && fractional_part_.to_string() != "0")
        {
            result.push_back('.');
...
inline bool operator<(const cait::numeric::pwrnum_t& lhs, const cait::numeric::pwrnum_t& rhs)
{
...
inline cait::numeric::pwrnum_t operator+(const cait::numeric::pwrnum_t& lhs, const cait::numeric::pwrnum_t& rhs)
{
    return lhs.add(rhs);
}
...
inline std::ostream& operator<<(std::ostream& os, const cait::numeric::pwrnum_t& number)
{
    os << number.to_string();
    return os;
}

inline cait::numeric::pwrnum_t operator""_pwrnum(const char* number, const size_t)
{
