...
#include "caitlyn/__testing/types/case.h"
...
public:
    ...
    using milliseconds_type = std::chrono::duration<long long, std::ratio<1, 1000>>;
...
public:
    ...
    void add_test(const std::string& suite_name, const std::string& test_name, const function_t func)
    {
    ...
    int run_all()
    {
        ...
        for (auto& suite : tests_)
        {
            ...
            for (auto& test : suite.second)
            {
                ...
                local_difference_ =
                    std::chrono::duration_cast<std::chrono::milliseconds>(stop_local_time_ - start_local_time_);
        ...
        global_difference_ =
            std::chrono::duration_cast<std::chrono::milliseconds>(stop_global_time_ - start_global_time_);
...
private:
    ...
    static void log_suite_start(const std::string& suite_name, const size_t test_count)
    {
    ...
    static void log_test_end(const case_t& test, const milliseconds_type& duration)
    {
        if (test.passed)
        {
    ...
    {
        std::cout << "[     FAIL ] " << test.suite_name << '.' << test.test_name << " (" << local_difference_.count()
                  << " ms)\n"
                  << "[     INFO ] " << message << "\n";
    }

    static void log_suite_end(
        const std::string& suite_name, const size_t test_count, const milliseconds_type& total_time)
    {
    ...
    void log_end() const
    {
        ...
        std::cout << "[==========] " << total_ << " tests from " << tests_.size() << " test cases ran. ("
                  << global_difference_.count() << " ms total)\n";
    }
