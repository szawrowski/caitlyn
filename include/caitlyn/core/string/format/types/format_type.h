// Copyright (c) 2024, Alexander Szawrowski
//
// This file is distributed under the MIT License.
// See LICENSE file for details.

#ifndef CAITLYN_CORE_STRING_FORMAT_TYPES_FORMAT_TYPE_H_
#define CAITLYN_CORE_STRING_FORMAT_TYPES_FORMAT_TYPE_H_

#include "caitlyn/core/string/format/defs/defs.h"

__caitlyn_begin_global_namespace
__caitlyn_begin_format_namespace

enum class format_type_t { string, integral, floating_point };

__caitlyn_end_format_namespace
__caitlyn_end_global_namespace

#endif  // CAITLYN_CORE_STRING_FORMAT_TYPES_FORMAT_TYPE_H_
