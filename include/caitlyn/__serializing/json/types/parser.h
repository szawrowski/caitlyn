...
private:
    parser_t(const string_type& json) : error_{error_t::no_error}, json_{json}, position_{}
    {
    }
    ...
    data_t parse_value()
    {
        ...
        if (current_char == def::latin_small_letter_t || current_char == def::latin_small_letter_f)
        {
        ...
        if (is_digit(current_char.get_codepoint()) || current_char == def::hyphen_minus ||
    ...
    data_t parse_string()
    {
        ...
        while (position_ < json_.size() && json_[position_] != def::quotation_mark)
        {
    ...
    data_t parse_number()
    {
        ...
        while (end_number < json_.size() &&
               (is_digit(json_[end_number].get_codepoint()) || json_[end_number] == def::full_stop ||
                json_[end_number] == def::latin_small_letter_e || json_[end_number] == def::latin_capital_letter_e ||
                json_[end_number] == def::hyphen_minus || json_[end_number] == def::plus_sign))
        {
            ++end_number;
        }
        const string_type number_str = json_.substr(position_, end_number - position_);
        try
        {
    ...
    void skip_ws()
    {
        while (position_ < json_.size() && is_space(json_[position_].get_codepoint()))
        {
            next();
        }
    }

    void next(const size_type count = 1)
    {
        position_ += count;
    }

    void set_error(const error_t error)
    {
        error_ = error;
    }
