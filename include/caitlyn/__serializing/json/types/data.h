...
public:
    ...
    data_t(const data_t& other) : type_(other.type_)
    {
        copy_union(other);
    }
    ...
    template <typename T>
    data_t(T value, typename std::enable_if<std::is_same<T, boolean_type>::value>::type* = nullptr)
    ...
    data_t(
        T value,
        typename std::enable_if<std::is_integral<T>::value && !std::is_same<T, boolean_type>::value>::type* = nullptr)
    ...
    template <typename T>
    data_t(T value, typename std::enable_if<std::is_floating_point<T>::value>::type* = nullptr)
    ...
    template <typename T>
    data_t(T value, typename std::enable_if<std::is_convertible<T, std::string>::value>::type* = nullptr)
...
public:
    ...
    template <typename T>
    typename std::enable_if<std::is_same<T, boolean_type>::value, data_t&>::type operator=(T value)
    {
    ...
    template <typename T>
    typename std::enable_if<std::is_integral<T>::value && !std::is_same<T, boolean_type>::value, data_t&>::type
    operator=(T value)
    {
    ...
    template <typename T>
    typename std::enable_if<std::is_floating_point<T>::value, data_t&>::type operator=(T value)
    {
    ...
    template <typename T>
    typename std::enable_if<std::is_convertible<T, std::string>::value, data_t&>::type operator=(T value)
    {
...
public:
    data_t& operator[](const string_type& key)
    {
        ...
        const auto it = std::find_if(
            object.begin(),
            object.end(),
            [&key](const std::pair<const string_type&, data_t*>& pair) { return pair.first == key; });
...
public:
    data_t& at(const string_type& key)
    {
        return operator[](key);
    }
    ...
    const data_t& at(const string_type& key) const
    {
        ...
        const auto it = std::find_if(
            object.begin(),
            object.end(),
            [&key](const std::pair<const string_type&, const data_t*>& pair) { return pair.first == key; });
    ...
    data_t& at(const size_type index)
    {
...
public:
    string_type str(const boolean_type mangling = false, const size_type indent = 2) const
    {
...
private:
    void make_output(
        std::ostringstream& oss, const boolean_type mangling, const size_type base_indent, const size_type indent) const
    {
            ...
            case class_t::object:
            {
                ...
                if (!object.empty())
                {
                    ...
                    for (const auto& member : object)
                    {
                        ...
                        oss << def::quotation_mark << member.first << def::quotation_mark << def::colon;
                        if (mangling)
                        {
                            oss << def::space;
                        }
                        member.second->make_output(oss, mangling, base_indent, indent + base_indent);
                        first = false;
                    }
                    if (mangling)
                    {
            ...
            case class_t::array:
            {
                ...
                if (!array.empty())
                {
                    ...
                    for (const auto& value : array)
                    {
                        ...
                        value->make_output(oss, mangling, base_indent, indent + base_indent);
                        first = false;
                    }
                    if (mangling)
                    {
    ...
    void move_union(data_t&& other)
    {
        switch (other.type_)
        {
            case class_t::null:
                ...
                break;
            case class_t::string:
                new (&data_.string_value) string_type{std::move(other.data_.string_value)};
                break;
            case class_t::floating:
                new (&data_.floating_value) floating_type(other.data_.floating_value);
                break;
            ...
            case class_t::array:
                ...
                break;
            case class_t::object:
                new (&data_.object_value) object_type{std::move(other.data_.object_value)};
                break;
            default:
                break;
        }
...
inline __detail::data_t make_object()
{
    return make(class_t::object);
}

inline __detail::data_t make_array()
{
...
inline __detail::data_t make_null()
{
...
inline __detail::data_t make_floating(const __detail::data_t::floating_type value)
{
    return __detail::data_t{std::to_string(value)};
}

inline __detail::data_t make_integral(const __detail::data_t::integral_type value)
{
    return __detail::data_t{std::to_string(value)};
}

inline __detail::data_t make_boolean(const __detail::data_t::boolean_type value)
{
    return __detail::data_t{std::to_string(value)};
}
