...
inline error::unexpected_t<basic_string_t<char>> make_failure(
    const char* error) {
  return error::unexpected_t<basic_string_t<char>>{basic_string_t<char>{error}};
}

inline error::unexpected_t<basic_string_t<char>> make_failure(
    const basic_string_t<char>& error) {
...
template <typename E>
bool operator==(const cait::error::unexpected_t<E>& lhs,
                const cait::error::unexpected_t<E>& rhs) {
