...
public:
    ...
    expected_t(expected_t&& other)
        noexcept(nothrow_move_constructible<value_type>() && nothrow_move_constructible<error_type>())
        : has_value_(other.has_value_)
    {
        if (has_value_)
        {
    ...
    expected_t& operator=(expected_t&& other)
        noexcept(nothrow_move_assignable<value_type>() && nothrow_move_assignable<error_type>())
    {
        if (this != &other)
        {
            destroy_union();
    ...
    auto map(Func func) const -> expected_t<typename std::result_of<Func(const value_type&)>::type, error_type>
    {
        using result_value_type = typename std::result_of<Func(const value_type&)>::type;
        if (has_value_)
        {
    ...
    template <typename Func>
    auto map_error(Func func) const -> expected_t<value_type, typename std::result_of<Func(const error_type&)>::type>
    {
        using result_error_type = typename std::result_of<Func(const error_type&)>::type;
        if (!has_value_)
        {
            return expected_t<value_type, result_error_type>(unexpected_t<result_error_type>(func(data_.error.get())));
        }
        return *this;
    }
    ...
    auto unwrap_or_else(ErrFunc func) const -> decltype(func(std::declval<unexpected_t<error_type>>().get()))
    {
        if (has_value_)
        {
            return data_.value;
    ...
    auto fold(U&& init, Func&& f) const -> decltype(f(std::declval<U>(), std::declval<value_type>()))
    {
        if (has_value_)
        {
    ...
    auto inspect(TFunc value_func, EFunc error_func, Args&&... args) const
        -> decltype(value_func(std::declval<value_type>(), std::forward<Args>(args)...))
    {
        if (has_value_)
        {
...
private:
    ...
    traits::required_t<destructible<Result>() && !destructible<Error>()> destroy_union()
    {
        if (has_value_)
        {
