...
inline void parse_view(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
...
inline void set_fill(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
    spec.fill = config[pos++];
}

// Parse fill character and alignment
inline void parse_alignment(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
    if (pos + 1 < config.size() &&
        (config[pos + 1] == def::less_than_sign || config[pos + 1] == def::greater_than_sign ||
...
inline void parse_width(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
...
inline void parse_precision(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
...
inline void parse_type(const basic_string_t<char>& config, spec_t& spec, size_t& pos)
{
    if (pos < config.size())
    {
