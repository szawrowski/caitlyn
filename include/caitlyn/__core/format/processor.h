...
inline void process_string_view(basic_string_t<char>& value, const spec_t& spec)
{
...
inline void process_integral_view(basic_string_t<char>& value, const spec_t& spec)
{
    auto to_binary = [](const basic_string_t<char>& str, const bool prefix)
    {
        std::ostringstream oss;
...
inline void process_floating_view(basic_string_t<char>& value, const spec_t& spec)
{
    auto to_binary = [](const basic_string_t<char>& str, const bool prefix)
    {
        std::ostringstream oss;
        ...
        const std::bitset<sizeof(double) * 8> bits{*reinterpret_cast<unsigned long long*>(&v)};
        oss << bits.to_string();
        return oss.str();
    };
    switch (spec.repr)
...
inline basic_string_t<char> process_string(const basic_string_t<char>& value, const spec_t& spec)
{
...
inline basic_string_t<char> process_floating(const basic_string_t<char>& value, const spec_t& spec)
{
    std::ostringstream oss;
    ...
    if (dot_index != basic_string_t<char>::npos)
    {
        ...
        if (spec.precision > 0)
        {
            ...
            while (index < value.size() && is_digit(value[index].get_codepoint()) && index < limit)
            {
                oss << value[index];
                ++index;
            }
...
inline basic_string_t<char> process_integral(const basic_string_t<char>& value, const spec_t& spec)
{
    basic_string_t<char> result = value;
    process_spec(result, spec);
    return result;
...
inline basic_string_t<char> process(const basic_string_t<char>& value, const spec_t& spec)
{
