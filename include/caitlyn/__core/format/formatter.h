...
basic_string_t<char> format(const basic_string_t<char>& formatter, Args&&... args)
{
    const std::vector<basic_string_t<char>> arguments{fmt::str(std::forward<Args>(args))...};
    ...
    while (pos < formatter.size())
    {
        if (formatter[pos] == def::left_curly_bracket)
        {
            ...
            else
            {
                ...
                if (spec.not_empty() && is_digit(spec[spec_begin].get_codepoint()))
                {
                    ...
                    while (index_end < spec.size() && is_digit(spec[index_end].get_codepoint()))
                    {
                        ++index_end;
                    }
                    try
                    {
                        arg_index = std::stoul(spec.substr(spec_begin, index_end - spec_begin).str());
                    }
        ...
        {
            if (pos + 1 < formatter.size() && formatter[pos + 1] == def::right_curly_bracket)
            {
