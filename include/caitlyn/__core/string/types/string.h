...
public:
    ...
    // Move constructor
    basic_string_t(basic_string_t&& other) noexcept : data_(std::move(other.data_))
    {
    }
...
public:
    ...
    char_type& operator[](const size_type index)
    {
        return data_[index];
    }
...
public:
    ...
    {
        const auto it =
            std::search(data_.begin() + static_cast<difference_type>(pos), data_.end(), substr.begin(), substr.end());
        return (it != data_.end()) ? std::distance(data_.begin(), it) : npos;
    }
    ...
    size_type rfind(const basic_string_t& substr, size_type pos = npos) const
    {
        ...
        const auto it = std::search(
            ...
            substr.rbegin(),
            substr.rend());
        return (it != data_.rend()) ? size() - 1 - std::distance(data_.rbegin(), it) : npos;
    }

    // Substring method
    basic_string_t substr(const size_type pos, const size_type count = npos) const
    {
        if (pos > size())
        {
        ...
        return basic_string_t{std::vector<char_type>(
            ...
            data_.begin() + static_cast<difference_type>(pos + std::min(count, size() - pos)))};
    }
    ...
    {
        auto it = std::find_if(data_.begin() + static_cast<difference_type>(pos), data_.end(), pred);
        return (it != data_.end()) ? std::distance(data_.begin(), it) : npos;
    }
    ...
    {
        auto it = std::find_if_not(data_.begin() + static_cast<difference_type>(pos), data_.end(), pred);
        return (it != data_.end()) ? std::distance(data_.begin(), it) : npos;
    }
...
public:
    ...
    void insert(const size_type pos, const char* cstr)
    {
        ...
        data_.insert(data_.begin() + static_cast<difference_type>(pos), temp_data.begin(), temp_data.end());
    }
    ...
    void insert(const size_type pos, const basic_string_t& str)
    {
        ...
        data_.insert(data_.begin() + static_cast<difference_type>(pos), str.data_.begin(), str.data_.end());
    }
...
inline std::istream& operator>>(std::istream& is, cait::basic_string_t<char>& str)
{
    if (is.good())
    {
...
inline std::ostream& operator<<(std::ostream& os, const cait::basic_string_t<char>& str)
{
    os << str.str();
    return os;
}

inline cait::basic_string_t<char> operator""_str(const char* str, const std::size_t)
{
