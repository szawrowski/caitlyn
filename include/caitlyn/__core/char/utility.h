...
inline codepoint_t to_uppercase(const codepoint_t codepoint) {
  ...
  if (codepoint >= 0x0100 && codepoint <= 0x012F) {
    if (codepoint % 2 == 0) return codepoint;
    return codepoint - 1;
  }
...
inline basic_character_t<char> to_uppercase(
    const basic_character_t<char>& value) {
  return to_uppercase(value.get_codepoint());
}

inline basic_character_t<char> to_lowercase(
    const basic_character_t<char>& value) {
